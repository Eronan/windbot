...
namespace WindBot.Game.AI.Decks
{
    ...
    public class AlbazExecutor : DefaultExecutor
    {
        public class CardId
        {
            ...
            public const int CalledByTheGrave = _CardId.CalledByTheGrave;
            public const int CrossoutDesignator = _CardId.CrossoutDesignator;
            public const int InfiniteImpermanence = _CardId.InfiniteImpermanence;
                        ...
                        if (hint == HintMsg.FusionMaterial && fusionTarget != null)
                        {
                            ...
                            if (fusionTarget.IsCode(albazFusionMonster))
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        List<ClientCard> targetList2 = cards.Where(c => func(c)).OrderBy(c => c.GetDefensePower()).ToList();
                                        if (targetList2.Count > 0)
                                        {
                                            selectedFusionMaterial.Add(targetList2[0]);
                                            return Util.CheckSelectCount(new List<ClientCard> { targetList2[0] }, cards, min, max);
                                        }
                                    }
                                }
                            }
